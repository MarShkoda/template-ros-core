...
class ImageLineProcessingOptimized:
    def __init__(self, width, height, color: ColorLine):
        ...
        if color == ColorLine.yellow:
            ...
            self.upper = np.array([179,254,254], np.uint8)
    ...
    def detect_line(self, binary):
        ...
        debug = False #self.pub_debug.get_num_connections() > 0
...
class ImageProcessingNode(DTROS):
    ...
    def callback(self, msg):
        ...
        if not valid:
            ...
            self.pub.publish(msg)
        rospy.loginfo("+++++++")
        rospy.loginfo("msg.omega {msg.omega} msg.v {msg.v}")
        rospy.loginfo("he {horizontal_error} ae {angular_error}")
